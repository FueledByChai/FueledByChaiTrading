...
public class HyperliquidUniverseParsingTest {
        ...
        public void testParseInstrumentDescriptors() {
                ...
                String universeJson = "{" + "\"universe\": [" + "  {" + "    \"name\": \"BTC\","
                                + "    \"szDecimals\": 5," + "    \"maxLeverage\": 50," + "    \"onlyIsolated\": false,"
                                + "    \"marginTableId\": null" + "  }," + "  {" + "    \"name\": \"ETH\","
                                + "    \"szDecimals\": 4," + "    \"maxLeverage\": 30," + "    \"onlyIsolated\": false,"
                                + "    \"marginTableId\": null" + "  }," + "  {" + "    \"name\": \"DELIST-COIN\","
                                + "    \"szDecimals\": 3," + "    \"maxLeverage\": 10," + "    \"onlyIsolated\": true,"
                                + "    \"marginTableId\": 1" + "  }" + "]" + "}";

                HyperliquidRestApi api = new HyperliquidRestApi("https://api.hyperliquid.xyz");
                InstrumentDescriptor[] descriptorsArray = api
                                .parseInstrumentDescriptors(InstrumentType.PERPETUAL_FUTURES, universeJson);
                ...
                // Test BTC descriptor
                InstrumentDescriptor btc = descriptors.stream().filter(d -> "BTC".equals(d.getCommonSymbol()))
                ...
                // Test ETH descriptor
                InstrumentDescriptor eth = descriptors.stream().filter(d -> "ETH".equals(d.getCommonSymbol()))
                                .findFirst().orElse(null);
        ...
        public void testParseInstrumentDescriptorsWithNullValues() {
                String jsonWithNulls = "{" + "\"universe\": [" + "  {" + "    \"name\": \"SOL\","
                                + "    \"szDecimals\": 3," + "    \"maxLeverage\": 20," + "    \"onlyIsolated\": false,"
                                + "    \"marginTableId\": null" + "  }" + "]" + "}";

                HyperliquidRestApi api = new HyperliquidRestApi("https://api.hyperliquid.xyz");
                InstrumentDescriptor[] descriptorsArray = api
                                .parseInstrumentDescriptors(InstrumentType.PERPETUAL_FUTURES, jsonWithNulls);
