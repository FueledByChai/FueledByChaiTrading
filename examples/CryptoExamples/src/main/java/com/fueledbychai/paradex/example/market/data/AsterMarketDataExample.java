...
public class AsterMarketDataExample {
    ...
    public void start(String symbol, InstrumentType instrumentType) throws InterruptedException {
        ...
        if (ticker == null) {
            throw new IllegalArgumentException("Unknown Aster ticker for " + instrumentType + ": " + symbol);
        }
        ...
        logger.info("Subscribed to Aster market data for {} ({})", ticker.getSymbol(), ticker.getInstrumentType());
    ...
    protected void onLevel1(ILevel1Quote quote) {
        logger.info("L1 {} bid={} ask={} mark={} underlying={} vol={} fundingHourlyBps={} fundingApr={}",
                quote.getTicker().getSymbol(), value(quote, QuoteType.BID), value(quote, QuoteType.ASK),
                value(quote, QuoteType.MARK_PRICE), value(quote, QuoteType.UNDERLYING_PRICE),
                value(quote, QuoteType.VOLUME), value(quote, QuoteType.FUNDING_RATE_HOURLY_BPS),
                value(quote, QuoteType.FUNDING_RATE_APR));

        // if (quote.containsType(QuoteType.FUNDING_RATE_HOURLY_BPS)) {
        // logger.info("Funding rate hourly bps: {}",
        // quote.getValue(QuoteType.FUNDING_RATE_HOURLY_BPS));
        // }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        new AsterMarketDataExample().start(symbol, instrumentType);
    }
}
